      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {shown.map((plan) => {
            ...
            const trf = plan.tarifications[idx];
            return (
              <div key={plan.id} className="bg-slate-900/50 border border-slate-850 rounded-2xl p-4 flex flex-col justify-between">
                <div>
                  <div className="flex items-start gap-2">
                    <span className="text-lg">{flagEmoji(plan.countryCode)}</span>
                    <p className="text-xs font-bold text-white leading-snug">{plan.name}</p>
                  </div>
                  <select
                    value={idx}
                    onChange={(e) => setSelDur({ ...selDur, [plan.id]: parseInt(e.target.value) })}
                    className="w-full mt-3 bg-slate-950 border border-slate-850 rounded-lg px-2.5 py-2 text-[11px] text-white focus:outline-none cursor-pointer"
                  >
                    {plan.tarifications.map((t: any, i: number) => (
                      <option key={i} value={i}>{durationLabel(t.time)} · {Math.round(t.trafficMb / 1024)} GB · ${t.priceUsd}</option>
                    ))}
                  </select>
                </div>
                <div className="flex items-center justify-between mt-3">
                  <span className="text-lg font-black text-white">${trf?.priceUsd ?? '—'}</span>
                  <button
                    onClick={() => order(plan)}
                    disabled={ordering === plan.id}
                    className="px-4 py-2 bg-gradient-to-r from-violet-600 to-fuchsia-600 hover:brightness-110 disabled:opacity-50 rounded-lg text-[11px] font-bold text-white flex items-center gap-1.5 cursor-pointer"
                  >
                    {ordering === plan.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Zap className="w-3.5 h-3.5" />}
                    {trf && walletBalance < trf.priceUsd ? 'Top Up' : 'Buy'}
                  </button>
                </div>
              </div>
            );
