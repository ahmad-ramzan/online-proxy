...
export const api = {
  ...
  mobile: {
    ...
    async checkout(planId: string, tarificationIndex: number, gateway: 'credit_card' | 'paystation' | 'cryptomus', custPhone?: string, couponCode?: string): Promise<{ checkoutUrl: string; transactionId: string; external?: boolean }> {
      const res = await fetch(`${API_BASE}/api/mobile/checkout`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ planId, tarificationIndex, gateway, custPhone, couponCode })
      });
      if (!res.ok) { const err = await res.json(); throw new Error(err.error || 'Mobile checkout failed'); }
      return res.json();
    },
    async order(planId: string, tarificationIndex: number, couponCode?: string): Promise<{ proxy: any }> {
      const res = await fetch(`${API_BASE}/api/mobile/order`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ planId, tarificationIndex, couponCode })
      });
